...
def create_name(times):
    last_name = ["赵", "钱", "孙", "李", "周", "吴", "郑", "王", "冯", "陈", "褚", "卫", "蒋", "沈", "韩", "杨", "朱", "秦", "尤", "许",
                 "吕",
                 "施", "张", "孔", "曹", "严", "华", "金", "魏", "陶", "姜", "戚", "谢", "邹", "喻", "柏", "水", "窦", "章", "云", "苏",
                 "潘",
                 "葛", "奚", "范", "彭", "郎", "鲁", "韦", "昌", "马", "苗", "凤", "花", "方", "俞", "任", "袁", "柳", "酆", "鲍", "史",
                 "唐",
                 "费", "廉", "岑", "薛", "雷", "贺", "倪", "汤", "滕", "殷", "罗", "毕", "郝", "邬", "安", "常", "乐", "于", "时", "傅",
                 "皮",
                 "卞", "齐", "康", "伍", "余", "元", "卜", "顾", "孟", "平", "黄", "和", "穆", "萧", "尹"]

    first_name = ['的', '一', '是', '了', '我', '不', '人', '在', '他', '有', '这', '个', '上', '们', '来', '到', '时', '大', '地',
    ...
    for i in range(times):
        name = (choice(last_name) + choice(first_name) + choice(first_name))
        return name, pinyin(name) + "@stu.kust.edu.cn"


def create_phone(times):
    begin = ['139', '138', '137', '136', '135', '134', '159', '158', '157', '150', '151', '152', '188', '187', '182',
             '183', '184', '178', '130', '131', '132', '156', '155', '186', '185', '176', '133', '153', '189', '180',
             '181', '177']
    number = '0123456789'
    for i in range(times):
        return choice(begin) + "".join(choice(number) for _ in range(8))
...
def create_dorm(times):
    park = ['憬园', '怡园', '恬园', '恒园', '莲华']
    number = '123456789'
    for i in range(times):
        return choice(park) + "".join(choice(number) for _ in range(4))


def create_sid(times):
    faculty = '1090'
    num_class = 1
    num_stu = 0
    ids = []
    for i in range(times):
        ...
        if num_stu == 51:
            num_stu = 1
            num_class += 1
        ids.append(faculty + str(num_class).zfill(2) + str(num_stu).zfill(2))
    return ids
    # return faculty + str(num_class).zfill(2) + str(num_stu).zfill(2)


def create_score(times):
    count = 0
    scores = []
    for i in range(times):
...
def create_info(times):
    id = create_sid(times)
    # id=[i for i in range(1001,2001)]
    score_1 = create_score(times)
    score_2 = create_score(times)
    score_3 = create_score(times)
    shuffle(score_1)
    shuffle(score_2)
    shuffle(score_3)
    result = []
    ...
    for i in range(times):
        name, email = create_name(times)
        phone = create_phone(times)
        dorm = create_dorm(times)
        # print(id[i], name, phone, email, dorm, score_1[i], score_2[i], score_3[i])
        result.append((id[i], name, phone, email, dorm, score_1[i], score_2[i], score_3[i]))
    return result


data = create_info(3000)
with open('stu_info.txt', 'w') as file:
    file.write('INSERT student(studentID,name,phone,email,dorm,score_1,score_2,score_3) VALUES\n')
    for i in data:
        file.write(str(i) + ',\n')
    file.close()
